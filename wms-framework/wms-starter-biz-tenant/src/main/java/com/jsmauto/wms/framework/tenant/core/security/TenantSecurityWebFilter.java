package com.jsmauto.wms.framework.tenant.core.security;
...
import com.jsmauto.wms.framework.web.core.filter.ApiRequestFilter;
...
public class TenantSecurityWebFilter extends ApiRequestFilter {
     ...
     * 允许忽略租户的 URL 列表
     *
     * 目的：解决 <a href="https://gitee.com/zhijiantianya/wms-cloud/issues/ICUQL9">修改配置会导致 @TenantIgnore Controller 接口过滤失效</>
     */
    private final Supplier<Set<String>> ignoreUrlsSupplier;

    private final AntPathMatcher pathMatcher;

    private final GlobalExceptionHandler globalExceptionHandler;
    private final TenantFrameworkService tenantFrameworkService;
    private volatile Set<String> ignoreUrls;

    public TenantSecurityWebFilter(WebProperties webProperties,
                                   TenantProperties tenantProperties,
                                   Supplier<Set<String>> ignoreUrlsSupplier,
                                   GlobalExceptionHandler globalExceptionHandler,
                                   TenantFrameworkService tenantFrameworkService) {
        super(webProperties);
        this.tenantProperties = tenantProperties;
        this.ignoreUrlsSupplier = ignoreUrlsSupplier;
    ...
    private boolean isIgnoreUrl(HttpServletRequest request) {
        ...
        Set<String> currentIgnoreUrls = getIgnoreUrls();
        // 快速匹配，保证性能
        if (CollUtil.contains(tenantProperties.getIgnoreUrls(), apiUri)
            || CollUtil.contains(currentIgnoreUrls, apiUri)) {
            return true;
        }
        // 逐个 Ant 路径匹配
        ...
        for (String url : currentIgnoreUrls) {
            if (pathMatcher.match(url, apiUri)) {
                return true;
            }
        }
    ...
    private Set<String> getIgnoreUrls() {
        Set<String> current = ignoreUrls;
        if (current != null) {
            return current;
        }
        synchronized (this) {
            if (ignoreUrls == null) {
                ignoreUrls = ignoreUrlsSupplier.get();
            }
            return ignoreUrls;
        }
    }
