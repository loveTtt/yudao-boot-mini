package com.jsmauto.wms.module.infra.service.codegen.inner;
...
import com.jsmauto.wms.framework.apilog.core.annotation.ApiAccessLog;
import com.jsmauto.wms.framework.apilog.core.enums.OperateTypeEnum;
...
import com.jsmauto.wms.framework.common.pojo.PageParam;
import com.jsmauto.wms.framework.common.pojo.PageResult;
import com.jsmauto.wms.framework.common.util.collection.CollectionUtils;
import com.jsmauto.wms.framework.common.util.date.DateUtils;
import com.jsmauto.wms.framework.common.util.date.LocalDateTimeUtils;
import com.jsmauto.wms.framework.common.util.object.BeanUtils;
import com.jsmauto.wms.framework.common.util.object.ObjectUtils;
import com.jsmauto.wms.framework.common.util.string.StrUtils;
import com.jsmauto.wms.framework.excel.core.annotations.DictFormat;
import com.jsmauto.wms.framework.excel.core.convert.DictConvert;
import com.jsmauto.wms.framework.excel.core.util.ExcelUtils;
import com.jsmauto.wms.framework.mybatis.core.dataobject.BaseDO;
...
import com.jsmauto.wms.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.jsmauto.wms.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.jsmauto.wms.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.jsmauto.wms.module.infra.enums.codegen.CodegenSceneEnum;
import com.jsmauto.wms.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.jsmauto.wms.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.jsmauto.wms.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 wms-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 wms-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.jsmauto.wms.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/wms-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/wms-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "wms-module-${table.moduleName}/" + // 顶级模块
                "wms-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "wms-module-${table.moduleName}/" + // 顶级模块
                "wms-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "wms-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "wms-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "wms-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "wms-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
